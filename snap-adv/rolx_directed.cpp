...
TIntFtrH CreateEmptyFeatures(const PNGraph Graph) {
...
TIntFtrH ExtractFeatures(const PNGraph Graph) {
...
void AddNeighborhoodFeatures(const PNGraph Graph, TIntFtrH& Features) {
...
void AddRecursiveFeatures(const PNGraph Graph, TIntFtrH& Features) {
...
void AddLocalFeatures(const PNGraph Graph, TIntFtrH& Features) {
  for (TNGraph::TNodeI NI = Graph->BegNI(); NI < Graph->EndNI(); NI++) {
    Features.GetDat(TInt(NI.GetId())).Add(NI.GetInDeg());
    Features.GetDat(TInt(NI.GetId())).Add(NI.GetOutDeg());
  }
}

void AddEgonetFeatures(const PNGraph Graph, TIntFtrH& Features) {
  for (TNGraph::TNodeI NI = Graph->BegNI(); NI < Graph->EndNI(); NI++) {
    int NId = NI.GetId();
    // int ArndEdges;
    ...
    PNGraph Egonet = TSnap::GetEgonet(Graph, NId, InEdges, OutEdges);
    Features.GetDat(NId).Add(Egonet->GetEdges());
    Features.GetDat(NId).Add(InEdges);
    Features.GetDat(NId).Add(OutEdges);
  }
}

TIntFtrH GenerateRecursiveFeatures(const PNGraph Graph,
...
TIntFtrH PruneRecursiveFeatures(const PNGraph Graph, const TIntFtrH& Features,
  ...
  PNGraph FeatureGraph = BuildFeatureGraph(LogBinFeatures,
      SimilarityThreshold);
...
PNGraph BuildFeatureGraph(const TIntFtrH& LogBinFeatures,
    const int SimilarityThreshold) {
  PNGraph FeatureGraph = PNGraph::New();
...
TIntFtrH SummarizeConnectedComponents(const PNGraph FeatureGraph,
...
void PlotRoles(const PNGraph Graph, const TIntIntH& Roles) {
